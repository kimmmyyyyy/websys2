...
class UserController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        // Persist computed fines for overdue transactions so users see values
        $overdueCandidates = BookTransaction::where('borrower_id', $borrower->id)
        ...
        // Currently borrowed (excluding overdue)
        $borrowedBooks = BookTransaction::where('borrower_id', $borrower->id)
            ->where('status', 'borrowed')
            ->with('book')
            ->get();

        // Include all transactions (borrowed and returned) in history and paginate
        $borrowingHistory = BookTransaction::where('borrower_id', $borrower->id)
            ->with('book')
            ->latest('created_at')
            ->paginate(10);

        // Overdue: include both 'borrowed' and 'overdue' statuses and only
        // transactions that haven't been returned yet.
        ...
        $overdueBooks = BookTransaction::where('borrower_id', $borrower->id)
            ->whereNull('return_date')
            ->where(function ($q) {
                $q->where('status', 'borrowed')
                  ->orWhere('status', 'overdue');
    ...
    public function requestBorrow(Request $request)
    {
        ...
        $dueDate = Carbon::now('Asia/Manila')->addDays(14)->setTime(23, 59, 0); // 14-day borrow period (end of day)
        ...
        $payload = [
            ...
            'book_id' => $book->id,
            'borrow_date' => $nowManila,
            'due_date' => $dueDate,
            'status' => 'borrowed',
        ];

        // Add borrow_time only if the column exists to avoid SQL errors on
        // databases that have not run the time-fields migration.
        if (Schema::hasColumn('book_transactions', 'borrow_time')) {
            $payload['borrow_time'] = $nowManila->toTimeString();
        }
